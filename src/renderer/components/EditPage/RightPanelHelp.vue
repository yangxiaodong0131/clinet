<template>
  <div id="edit-rightpanelhelp">
    ...
    <table v-if="rightPanels.includes('编辑器使用帮助') && !this.$store.state.Edit.rightFolds.includes('编辑器使用帮助')">
      ...
      <tr>
        <td>
          一、病案的半结构化：病案数据分为5层结构
          <ul>
            <li>病案集合<em class="table-primary">（文件）</em>：单条或多条病案保存为一个CDA文件（数据库表），每一行是一条病案</li>
      ...
      <tr>
        <td>
          ...
          <ul>
            ...
            <li>
              术语字典（标准化）：病案条目的key或value的可选择项，例如疾病诊断名称、手术/操作名称、性别、体态、步态、面容、等等
            </li>
          </ul>
        </td>
      </tr>
        ...
        <td>
          ...
          <ul>
            <li class="text-danger">
              在底部输入框中每次输入一个条目，用空格分隔条目中的内容结构，回车确定输入。内容中如果包含中（英）文逗号，则自动拆分成多个条目输入。
            </li>
            <li>
            ...
            <li>
              用ctrl+上、下键可以移动条目在文档中的位置
            </li>
            <li>
              用ctrl+delete键可以删除条目
            </li>
            <li>
              用ctrl+enter键，在文档中当前位置后插入条目
            </li>
            <li>
            ...
            <li>
              左菜单中的《新建》《保存》《另存为模板》都是对文件中的一条（行）记录的操作。
            </li>
            <li>
              右菜单中的《本地》《远程》《区块链》分别是打开本地数据库、远程数据库、区块链数据库上的文件
            </li>
            <li>
              右菜单中的《用户》《客户》《文档》《模板》分别是打开各自目录下的文件
            </li>

            <li>左工具栏中的《新建》可以选择不同的模板，默认是《自定义文档》，也就是空白文档。</li>

            <li>右工具栏中的《帮助》可以选择不同的帮助页面，默认是《编辑器使用帮助》，也就是当前看到的帮助。</li>
          </ul>
        </td>
      </tr>
    </table>
    ...
    <table v-if="rightPanels.includes('在线交流')">
      <th colspan="10" class="table-info">在线交流--<a href="#">联系人</a>--<a href="#">消息</a></th>
