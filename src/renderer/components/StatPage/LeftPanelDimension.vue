    ...
    methods: {
      setDimension: function (data, index) {
        ...
        if (this.$store.state.Stat.tableType === 'server') {
          ...
          getStat(this, [this.$store.state.System.server, this.$store.state.System.port], { tableName: this.$store.state.Stat.serverTable.tableName, page: 1, username: this.$store.state.System.user.username, dimension: this.$store.state.Stat.serverTableDimension }, 'stat')
