    ...
    <div>
      <table>
        ...
        <tr v-for="(data, indexs) in xs" v-bind:key='indexs' class="stat-right-table-tr" v-bind:class="{'table-danger':flag.find((n)=>n===indexs)}" v-if="indexs > 0">
          ...
          <td v-for="(field, index) in data"  v-bind:key='index' v-bind:class="{'table-danger':flagTd.find((n)=>n===index)}" class="stat-right-table-td"  v-if="index >= 1 && index < 11">{{data[index]}}</td>
