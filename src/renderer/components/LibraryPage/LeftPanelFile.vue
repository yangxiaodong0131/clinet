    ...
    methods: {
      loadFile: function (data, index) {
        ...
        if (this.$store.state.Library.tableType === 'server') {
          ...
          getLibrary(this, [this.$store.state.System.server, this.$store.state.System.port], data, 1, null, null, 'library', 'server', ['asc', ''])
        } else if (this.$store.state.Library.tableType === 'block') {
          this.$store.commit('LIBRARY_SET_TABLE_PAGE', 1);
          getLibrary(this, [this.$store.state.System.server, this.$store.state.System.port], data, 1, null, null, 'library', 'block', ['asc', ''])
        } else {
